...
-- VIEWS
--
-- The total is denormalised onto guides for the same reason votes are:
-- the accessors that render a guide are synchronous over a cache and
-- cannot run a COUNT.
-- ============================================================

alter table public.guides add column if not exists views integer not null default 0;

-- What counts as a view is decided here, not in the client: one viewer,
-- one guide, one day, enforced by the primary key. Refreshing your own
-- guide cannot pump the number; a reader returning next week is counted
-- again, which is the thing an author actually wants to know.
--
-- `day` comes from a DEFAULT rather than the request, so a client cannot
-- backdate its way around the key. `viewer` is an auth uid for a signed-in
-- reader and an opaque random per-browser id otherwise.
create table if not exists public.guide_views (
  guide_slug text not null references public.guides(slug) on delete cascade,
  viewer     text not null,
  day        date not null default (now() at time zone 'utc')::date,
  primary key (guide_slug, viewer, day)
);

alter table public.guide_views enable row level security;

-- INSERT only, with no select policy and no select grant. This table is a
-- record of who read what; the public needs the total, not the rows behind
-- it. The aggregate on guides.views is public, this is not.
drop policy if exists views_insert on public.guide_views;
create policy views_insert on public.guide_views for insert
  with check (
    (select auth.uid()) is null or viewer = (select auth.uid())::text
  );

-- The table is closed to the public entirely — no select, no insert. The
-- only way in is record_guide_view() below.
--
-- The first version granted INSERT and had the client send
-- Prefer: resolution=ignore-duplicates so the primary key absorbed a
-- repeat view. That fails: PostgREST implements it with ON CONFLICT, and
-- ON CONFLICT requires SELECT on the target table. It says so in the error
-- — "GRANT SELECT ON public.guide_views" — and that grant is the one thing
-- this table must not have.
revoke all on public.guide_views from anon, authenticated;

-- Counting a read, as the owner, so the conflict handling works without
-- opening the rows. The viewer identity is decided here rather than
-- trusted from the request: a signed-in caller is always counted as
-- themselves, and p_anon is consulted only when there is no session.
create or replace function public.record_guide_view(p_slug text, p_anon text default null)
returns void
language plpgsql
security definer
...
declare v text;
begin
  v := coalesce((select auth.uid())::text, nullif(btrim(p_anon), ''));
  if v is null then return; end if;
  if p_slug is null or btrim(p_slug) = '' then return; end if;
  v := left(v, 64);                     -- not free storage for a caller
  insert into public.guide_views (guide_slug, viewer)
  values (p_slug, v)
  on conflict do nothing;
end $$;

-- This one IS meant to be callable by the public, unlike the trigger
-- functions elsewhere in this file. It is the whole write path.
--
-- Supabase's linter warns about it twice — "Public Can Execute SECURITY
-- DEFINER Function" and the signed-in equivalent. Both are expected and
-- both are accepted. The warning exists because a definer function running
-- as the owner bypasses RLS, and that is precisely what this one is for:
-- writing a row into a table nobody is allowed to read. It is safe to
-- expose because of what it CANNOT do — it takes two text arguments,
-- returns nothing, writes one bounded row into one table, takes the
-- signed-in identity from the session instead of the request, and has no
-- branch that touches anything else.
--
-- Do not "fix" this by revoking EXECUTE. That silently stops every view
-- being counted, with no error anywhere, because recordView() is
-- deliberately quiet about failure.
grant execute on function public.record_guide_view(text, text) to anon, authenticated;

-- The honest limitation: an anonymous viewer id is generated by the
-- browser, so someone determined can send fresh ids and inflate a count.
-- Nothing client-side can prevent that with a publishable key. The RLS
-- above at least binds a SIGNED-IN reader to their own id, and the per-day
-- key stops the ordinary accident of a refresh counting twice. If view
-- counts ever start deciding anything (ranking, payment, prizes), they
-- need to be counted somewhere the client cannot reach.

create or replace function public.sync_view_count() returns trigger
language plpgsql security definer set search_path = public as $$
begin
  update public.guides set views = views + 1 where slug = new.guide_slug;
  return new;
end $$;

drop trigger if exists views_count on public.guide_views;
create trigger views_count after insert on public.guide_views
  for each row execute function public.sync_view_count();

revoke execute on function public.sync_view_count() from anon, authenticated, public;

-- Nothing prunes guide_views. One row per reader per guide per day is
